...
export class ActivityPanel {
  ...
  formatEvent(event) {
    ...
    if (event.payload?.mobileChat === 'submitted') return `Mobile: ${event.payload.preview || 'prompt sent'}`;
    if (event.payload?.mobileChat === 'completed') return `Mobile reply: ${event.payload.preview || 'done'}`;
    if (event.payload?.mobileChat === 'error') return `Mobile error: ${event.payload.error || 'failed'}`;
  ...
  formatOrigin(event) {
    ...
    if (event.payload?.mobileChat) return 'mobile';
  ...
  formatTone(event) {
    if (event.payload?.mobileChat === 'error') return 'critical';
    if (event.payload?.mobileChat === 'submitted') return 'ok';
    if (event.payload?.mobileChat === 'completed') return 'ok';
